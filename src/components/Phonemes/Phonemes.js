    ...
    <article className="phonemes-article">
      ...
      <div className={`phonemList ${type}`}>
        {items.map(({ phonem, word }) => {
          return (
            <Phoneme key={`${phonem}-${word}`} phonem={phonem} word={word} />
          );
        })}
      </div>
    </article>
...
function Phoneme({ phonem, word }) {
  ...
  return (
    <div className="pair">
      {phonemFile && (
        <>
          <span className="phonem" onClick={handlePhonemeClick}>
            {phonem}
          </span>
          <audio ref={phonemeRef} hidden={true} controls>
            <source src={phonemFile} type="audio/mpeg"></source>
      ...
      {wordFile && (
        <>
          <span className="phonem-word" onClick={handleWordClick}>
            {word}
          </span>
